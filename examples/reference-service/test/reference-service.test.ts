 ...
 * Integration tests используют только вымышленные fixtures.
 */
...
describe("reference service", () => {
  ...
  test("повтор idempotency key не создаёт record и outbox event повторно", async () => {
    const request = {
      method: "POST" as const,
      url: "/v1/spaces/space-alpha/records",
      headers: { "x-user-id": "user-owner", "idempotency-key": "create-1" },
      payload: { title: "Проверяемая запись" },
    };
    ...
    const conflict = await app.inject({ ...request, payload: { title: "Другой payload" } });
    assert.equal(conflict.statusCode, 409);
    assert.deepEqual(conflict.json(), { error: "idempotency_key_reused" });
  });

  test("конкурентные запросы с одним ключом создают один результат", async () => {
    const request = {
      method: "POST" as const,
      url: "/v1/spaces/space-alpha/records",
      headers: { "x-user-id": "user-owner", "idempotency-key": "concurrent-1" },
      payload: { title: "Конкурентная запись" },
    };
  ...
  test("один ключ нельзя повторно использовать в другом space", async () => {
    await pool.query(
  ...
  test("worker обрабатывает durable event ровно один раз", async () => {
    await app.inject({
      method: "POST",
      url: "/v1/spaces/space-alpha/records",
      headers: { "x-user-id": "user-owner", "idempotency-key": "worker-1" },
      payload: { title: "Событие для worker" },
    });
  ...
  test("ошибка handler откатывает side effects и планирует retry", async () => {
    await app.inject({
      method: "POST",
      url: "/v1/spaces/space-alpha/records",
